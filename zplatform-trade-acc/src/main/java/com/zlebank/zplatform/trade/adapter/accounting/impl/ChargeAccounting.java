...
public class ChargeAccounting implements IAccounting {
	...
	public ResultBean commonTradeAccounting(TxnsLogModel txnsLog){
		...
		try {
			...
			if ("0000".equals(retCode)) {
				...
				txnsLog.setApporderstatus(AccStatusEnum.AccountingFail
						.getCode());
			}
		...
		txnsLogService.updateAppStatus(txnsLog.getTxnseqno(), txnsLog.getApporderstatus(),
	...
	public ResultBean industryTradeAccounting(TxnsLogModel txnsLog){
		...
		try {
			...
			txnsLog.setAppinst("000000000000");// 没实际意义，可以为空
			if ("0000".equals(retCode)) {
				txnsLog.setApporderinfo("行业专户充值入账成功");
				txnsLog.setApporderstatus(AccStatusEnum.Finish.getCode());
			} else {
				txnsLog.setApporderinfo(retInfo);
				txnsLog.setApporderstatus(AccStatusEnum.AccountingFail
